...
DecodePs::DecodePs(WindowBuf *pWBuf, ILog *pLog) :
    m_pWBuf(pWBuf),
    m_pLog(pLog) {
    // HACK
...
//
bool DecodePs::FindBimRecord(uint32_t nBimId, uint32_t &nFldInd) {
    uint32_t nInd = 0;
...
//
bool DecodePs::LookupIptcField(uint32_t nRecord, uint32_t nDataSet, uint32_t &nFldInd) {
    uint32_t nInd = 0;
...
//
QString DecodePs::DecodeIptcValue(teIptcType eIptcType, uint32_t nFldCnt, uint32_t nPos) {
    //uint32_t      nFldInd = 0;
    uint32_t nInd;
    uint32_t nVal;
    ...
    switch (eIptcType) {
        ...
        case IPTC_T_NUM1:
        case IPTC_T_NUM2:
            nVal = m_pWBuf->getDataX(nPos, nFldCnt, PS_BSWAP);
            nPos += nFldCnt;
            nPos += nFldCnt;
    ...
    while (!bDone) {
        ...
        if (nTagMarker == 0x1C) {
            ...
            strDebug = QString("## File=[%1] Block=[%2] Error=[%3]\n")
                .arg(m_pAppConfig->curFileName, -100)
                .arg("PsDecode", -10)
                .arg(strTmp);
            qDebug() << (strDebug);
#endif
...
QString DecodePs::PhotoshopParseIndent(uint32_t nIndent) {
    QString strIndent = "";
...
QString DecodePs::PhotoshopParseGetLStrAsc(uint32_t &nPos) {
    uint32_t nStrLen;

    QString strVal = "";

    nStrLen = m_pWBuf->getData4(nPos, PS_BSWAP);

    if (nStrLen != 0) {
        strVal = m_pWBuf->readStrN(nPos, nStrLen);
        nPos += nStrLen;
    } else {
        // 4 byte string
...
// RETURN:
// - Unicode string
QString DecodePs::PhotoshopParseGetBimLStrUni(uint32_t nPos, uint32_t &nPosOffset) {
    QString strVal;
    ...
    nPosOffset = 0;
    nStrLenActual = m_pWBuf->getDataX(nPos + nPosOffset, 4, PS_BSWAP);
    nPosOffset += 4;
...
void DecodePs::PhotoshopParseReportNote(uint32_t nIndent, QString strNote) {
    QString strIndent;
    QString strLine;
...
void DecodePs::PhotoshopParseReportFldNum(uint32_t nIndent, QString strField, uint32_t nVal, QString strUnits) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void DecodePs::PhotoshopParseReportFldBool(uint32_t nIndent, QString strField, uint32_t nVal) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
//
void DecodePs::PhotoshopParseReportFldHex(uint32_t nIndent, QString strField, uint32_t nPosStart, uint32_t nLen) {
    QString strIndent;
    QString strPrefix;
    QString strByteHex;
    QString strByteAsc;
        ...
        if (nRowOffset >= nLenClip) {
            ...
            for (uint32_t nInd = 0; nInd < PS_HEX_MAX_ROW; nInd++) {
                if ((nRowOffset + nInd) < nLenClip) {
                    nByte = m_pWBuf->getByte(nPosStart + nRowOffset + nInd);
...
QString DecodePs::PhotoshopDispHexWord(uint32_t nVal) {
    QString strValHex;
    QString strValAsc;
    QString strByteHex;
    QString strByteAsc;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void
DecodePs::PhotoshopParseReportFldEnum(uint32_t nIndent, QString strField, teBimEnumField eEnumField, uint32_t nVal) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void DecodePs::PhotoshopParseReportFldFixPt(uint32_t nIndent, QString strField, uint32_t nVal, QString strUnits) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void DecodePs::PhotoshopParseReportFldFloatPt(uint32_t nIndent, QString strField, uint32_t nVal, QString strUnits) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void DecodePs::PhotoshopParseReportFldDoublePt(uint32_t nIndent, QString strField, uint32_t nVal1, uint32_t nVal2,
                                               QString strUnits) {
    QString strIndent;
    QString strVal;
    QString strLine;
...
// - Report the value with the field name (strField) and current indent level (nIndent)
void DecodePs::PhotoshopParseReportFldStr(uint32_t nIndent, QString strField, QString strVal) {
    QString strIndent;
    QString strLine;
...
void DecodePs::PhotoshopParseReportFldOffset(uint32_t nIndent, QString strField, uint32_t nOffset) {
    QString strIndent;
    QString strLine;
...
void DecodePs::PhotoshopParseThumbnailResource(uint32_t &nPos, uint32_t nIndent) {
    uint32_t nVal;
...
//
void DecodePs::PhotoshopParseVersionInfo(uint32_t &nPos, uint32_t nIndent) {
    uint32_t nVal;
...
void DecodePs::PhotoshopParsePixelAspectRatio(uint32_t &nPos, uint32_t nIndent) {
    ...
    nVal = m_pWBuf->getData4(nPos, PS_BSWAP);
    PhotoshopParseReportFldNum(nIndent, "Version", nVal, "");
    nVal1 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nVal2 = m_pWBuf->getData4(nPos, PS_BSWAP);
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseGridGuides(uint32_t &nPos, uint32_t nIndent) {
    uint32_t nVal;
    ...
    uint32_t nNumGuides = m_pWBuf->getData4(nPos, PS_BSWAP);
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseLayerGroupInfo(uint32_t &nPos, uint32_t nIndent, uint32_t nLen) {
    uint32_t nVal;
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseLayerGroupEnabled(uint32_t &nPos, uint32_t nIndent, uint32_t nLen) {
    uint32_t nVal;
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseFileHeader(uint32_t &nPos, uint32_t nIndent, tsImageInfo *psImageInfo) {
    ...
    QString strSig = m_pWBuf->readStrN(nPos, 4);
...
void DecodePs::PhotoshopParseColorModeSection(uint32_t &nPos, uint32_t nIndent) {
    ...
    uint32_t nColModeLen = m_pWBuf->getData4(nPos, PS_BSWAP);
...
bool DecodePs::PhotoshopParseLayerMaskInfo(uint32_t &nPos, uint32_t nIndent, QImage *pDibTemp) {
    ...
    uint32_t nLayerMaskLen = m_pWBuf->getData4(nPos, PS_BSWAP);
...
bool DecodePs::PhotoshopParseLayerInfo(uint32_t &nPos, uint32_t nIndent, QImage *pDibTemp) {
    ...
    uint32_t nLayerLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    uint16_t nLayerCountU = m_pWBuf->getData2(nPos, PS_BSWAP);
...
// - nPos               = File position after reading the block
//
bool DecodePs::PhotoshopParseLayerRecord(uint32_t &nPos, uint32_t nIndent, tsLayerInfo *pLayerInfo) {
    QString strVal;
    ...
    nRect1 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRect2 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRect3 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRect4 = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    uint32_t nNumChans = m_pWBuf->getData2(nPos, PS_BSWAP);
    ...
    for (uint32_t nChanInd = 0; nChanInd < nNumChans; nChanInd++) {
        uint32_t nChanID = m_pWBuf->getData2(nPos, PS_BSWAP);

        uint32_t nChanDataLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    QString strBlendModeSig = m_pWBuf->readStrN(nPos, 4);

    nPos += 4;
    PhotoshopParseReportFldStr(nIndent, "Blend mode signature", strBlendModeSig);
    uint32_t nBlendModeKey = m_pWBuf->getData4(nPos, PS_BSWAP);

    PhotoshopParseReportFldEnum(nIndent, "Blend mode key", BIM_T_ENUM_BLEND_MODE_KEY, nBlendModeKey);
    uint32_t nOpacity = m_pWBuf->getData1(nPos, PS_BSWAP);

    PhotoshopParseReportFldNum(nIndent, "Opacity", nOpacity, "(0=transparent ... 255=opaque)");
    m_pWBuf->getData1(nPos, PS_BSWAP);   // unsigned nClipping
    m_pWBuf->getData1(nPos, PS_BSWAP);   // unsigned nFlags
    m_pWBuf->getData1(nPos, PS_BSWAP);   // unsigned nFiller
    uint32_t nExtraDataLen = m_pWBuf->getData4(nPos, PS_BSWAP);
...
bool DecodePs::PhotoshopParseLayerMask(uint32_t &nPos, uint32_t nIndent) {
    ...
    nRectA1 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRectA2 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRectA3 = m_pWBuf->getData4(nPos, PS_BSWAP);
    nRectA4 = m_pWBuf->getData4(nPos, PS_BSWAP);

    uint32_t nTmp;

    m_pWBuf->getData1(nPos, PS_BSWAP);   // unsigned nDefaultColor
    uint32_t nFlags = m_pWBuf->getData1(nPos, PS_BSWAP);

    //FIXME: Is this correct?
    if (nLayerMaskLen == 20) {
        m_pWBuf->getData2(nPos, PS_BSWAP); // unsigned nPad
    }

    if (nFlags & (1 << 4)) {
        uint32_t nMaskParams = m_pWBuf->getData1(nPos, PS_BSWAP);

        if (nMaskParams & (1 << 0)) {
            // User mask density, 1 byte
            nTmp = m_pWBuf->getData1(nPos, PS_BSWAP);
        }

        if (nMaskParams & (1 << 1)) {
            // User mask feather, 8 bytes, double
            nTmp = m_pWBuf->getData4(nPos, PS_BSWAP);
            nTmp = m_pWBuf->getData4(nPos, PS_BSWAP);
        }
        ...
        m_pWBuf->getData2(nPos, PS_BSWAP); // unsigned nPadding
        m_pWBuf->getData1(nPos, PS_BSWAP); // unsigned nRealFlags
        m_pWBuf->getData1(nPos, PS_BSWAP); // unsigned nRealUserMaskBackground
        nRectB1 = m_pWBuf->getData4(nPos, PS_BSWAP);
        nRectB2 = m_pWBuf->getData4(nPos, PS_BSWAP);
        nRectB3 = m_pWBuf->getData4(nPos, PS_BSWAP);
        nRectB4 = m_pWBuf->getData4(nPos, PS_BSWAP);
    }
...
bool DecodePs::PhotoshopParseLayerBlendingRanges(uint32_t &nPos, uint32_t nIndent) {
    ...
    uint32_t nLayerBlendLen = m_pWBuf->getData4(nPos, PS_BSWAP);

    if (nLayerBlendLen == 0) {
        return bDecOk;
    }

    m_pWBuf->getData4(nPos, PS_BSWAP);   // unsigned nCompGrayBlendSrc
    m_pWBuf->getData4(nPos, PS_BSWAP);   // unsigned nCompGrayBlendDstRng
    uint32_t nNumChans;

    nNumChans = (nLayerBlendLen - 8) / 8;

    for (uint32_t nChanInd = 0; nChanInd < nNumChans; nChanInd++) {
        m_pWBuf->getData4(nPos, PS_BSWAP); // unsigned nSrcRng
        m_pWBuf->getData4(nPos, PS_BSWAP); // unsigned nDstRng
    }
...
// - nPos               = File position after reading the block
//
bool DecodePs::PhotoshopParseChannelImageData(uint32_t &nPos, uint32_t nIndent, uint32_t nWidth, uint32_t nHeight,
                                              uint32_t nChan, unsigned char *pDibBits) {
    bool bDecOk = true;

    uint32_t nCompressionMethod = m_pWBuf->getData2(nPos, PS_BSWAP);
    ...
    if (nCompressionMethod == 1) {
        ...
        for (uint32_t nRow = 0; nRow < nHeight; nRow++) {
            anRowLen[nRow] = m_pWBuf->getData2(nPos, PS_BSWAP);
        }
        ...
        m_pLog->warn("Unsupported compression method. Stopping.");
        bDecOk = false;
        return bDecOk;
    }
...
bool DecodePs::PhotoshopDecodeRowRle(uint32_t &nPos, uint32_t nWidth, uint32_t nHeight, uint32_t nRow, uint32_t nRowLen,
    ...
    while ((bDecOk) && (nRowOffsetComp < nRowLen)) {
        ...
        nRleRun = m_pWBuf->getData1(nPos, PS_BSWAP);
...
//
bool DecodePs::PhotoshopParseImageData(uint32_t &nPos, uint32_t nIndent, tsImageInfo *psImageInfo,
                                       unsigned char *pDibBits) {
    Q_ASSERT(psImageInfo);
...
bool DecodePs::PhotoshopParseGlobalLayerMaskInfo(uint32_t &nPos, uint32_t nIndent) {
    ...
    uint32_t nInfoLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    m_pWBuf->getData4(nPos, PS_BSWAP);   // unsigned nOverlayColorSpace
    m_pWBuf->getData2(nPos, PS_BSWAP);   // unsigned nColComp1
    m_pWBuf->getData2(nPos, PS_BSWAP);   // unsigned nColComp2
    m_pWBuf->getData2(nPos, PS_BSWAP);   // unsigned nColComp3
    m_pWBuf->getData2(nPos, PS_BSWAP);   // unsigned nColComp4
    m_pWBuf->getData2(nPos, PS_BSWAP);   // unsigned nOpacity
    m_pWBuf->getData1(nPos, PS_BSWAP);   // unsigned nKind
...
// - nPos               = File position after reading the block
//
bool DecodePs::PhotoshopParseAddtlLayerInfo(uint32_t &nPos, uint32_t nIndent) {
    bool bDecOk = true;
    ...
    if (strSig != "8BIM") {
        ...
        strError = QString("ERROR: Addtl Layer Info signature unknown [%1] @ 0x%2").arg(strSig).arg(nPos - 4,
                                                                                                    8,
                                                                                                    16,
                                                                                                    QChar('0'));
        ...
        strDebug =
            QString("## File=[%1] Block=[%2] Error=[%3]\n").arg(m_pAppConfig->curFileName, -100).arg("PsDecode",
                                                                                                     -10).arg(strError);
        qDebug() << (strDebug);
#else
        Q_ASSERT(false);
    ...
    QString strKey = m_pWBuf->readStrN(nPos, 4);
   ...
   */
    ...
    if (!bLen8B) {
        nLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    } else {
        // DUMMY for now
        //FIXME: Untested
        nLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    if (strKey == "luni") {
        ...
        nVal = m_pWBuf->getDataX(nPos, 4, PS_BSWAP);
...
bool DecodePs::PhotoshopParseImageResourcesSection(uint32_t &nPos, uint32_t nIndent) {
    ...
    uint32_t nImgResLen = m_pWBuf->getData4(nPos, PS_BSWAP);
...
// - nPos               = File position after reading the block
//
bool DecodePs::PhotoshopParseImageResourceBlock(uint32_t &nPos, uint32_t nIndent) {
    QString strVal;
    ...
    uint32_t nBimLen = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    if (nBimLen == 0) {
        ...
        switch (asBimRecords[nFldInd].eBimType) {
            ...
            case BIM_T_PS_STR_ASC:
                strVal += m_pWBuf->readStrN(nPos, nBimLen);
                nPos += nBimLen;
                PhotoshopParseReportFldStr(nIndent, strBimDefName, strVal);
                break;
        ...
        if (nPos > nPosEnd + 1) {
            ...
            strTmp = QString(
                "ERROR: Parsing exceeded expected length. Stopping decode. BIM=[%1], CurPos=[0x%2], ExpPosEnd=[0x%3], ExpLen=[%4]")
                .arg(strBimDefName)
                .arg(nPos, 8, 16, QChar('0'))
            ...
            strTmp = QString(
                "WARNING: Parsing offset length mismatch. Current pos=[0x%1], expected end pos=[0x%2], expect length=[%3]")
                .arg(nPos, 8, 16, QChar('0'))
                .arg(nPosEnd + 1, 8, 16, QChar('0'))
                .arg(nBimLen);
            m_pLog->warn(strTmp);
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseSliceHeader(uint32_t &nPos, uint32_t nIndent, uint32_t nPosEnd) {
    uint32_t nVal;
    ...
    uint32_t nSliceVer = m_pWBuf->getData4(nPos, PS_BSWAP);
    ...
    if (nSliceVer == 6) {
        ...
        uint32_t nNumSlices = m_pWBuf->getData4(nPos, PS_BSWAP);
...
//
void DecodePs::PhotoshopParseSliceResource(uint32_t &nPos, uint32_t nIndent, uint32_t nPosEnd) {
    uint32_t nVal;
    ...
    uint32_t nOrigin = m_pWBuf->getData4(nPos, PS_BSWAP);
...
//
void DecodePs::PhotoshopParseJpegQuality(uint32_t &nPos, uint32_t nIndent, uint32_t) {
    uint32_t nVal;
    ...
    // Index 1: Format
    nSaveFormat = m_pWBuf->getData2(nPos, PS_BSWAP);
...
// - nPos               = File position after reading the block
//
void DecodePs::PhotoshopParseHandleOsType(QString strOsType, uint32_t &nPos, uint32_t nIndent) {
    if (strOsType == "obj ") {
...
//
void DecodePs::PhotoshopParseDescriptor(uint32_t &nPos, uint32_t nIndent) {
    QString strVal;
    ...
    uint32_t nDescNumItems = m_pWBuf->getData4(nPos, PS_BSWAP);
...
//
void DecodePs::PhotoshopParseStringUni(uint32_t &nPos, uint32_t nIndent) {
    QString strVal;
