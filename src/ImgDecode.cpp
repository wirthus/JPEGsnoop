...
//
void ImgDecode::SetPreviewYccOffset(uint32_t nMcuX, uint32_t nMcuY, int32_t nY, int32_t nCb, int32_t nCr) {
    m_nPreviewShiftY = nY;
...
// - nCr                                = DC shift in Cr component
//
void ImgDecode::GetPreviewYccOffset(uint32_t &nMcuX, uint32_t &nMcuY, int32_t &nY, int32_t &nCb, int32_t &nCr) {
    nY = m_nPreviewShiftY;
...
// UNUSED
void ImgDecode::SetPreviewMcuInsert(uint32_t nMcuX, uint32_t nMcuY, int32_t nLen) {
...
// UNUSED
void ImgDecode::GetPreviewMcuInsert(uint32_t &nMcuX, uint32_t &nMcuY, uint32_t &nLen) {
...
//
void ImgDecode::GetPreviewPos(uint32_t &nX, uint32_t &nY) {
    nX = m_nPreviewPosX;
    nY = m_nPreviewPosY;
}
...
//
void ImgDecode::GetPreviewSize(uint32_t &nX, uint32_t &nY) {
    nX = m_nPreviewSizeX;
    nY = m_nPreviewSizeY;
}
...
//
void ImgDecode::CalcChannelPreviewFull(QRect *, uint8_t *pTmp) {
    _log.debug("ImgDecode::CalcChannelPreviewFull");

    if (!_imgDecoded) {
        _log.debug("ImgDecode::CalcChannelPreviewFull exit as image not decoded");
        return;
    }
    ...
    if (m_bDetailVlc) {
        _log.info("  Detailed IDCT Dump (RGB):");
    ...
    _log.debug(QString("ImgDecode::CalcChannelPreviewFull() new QImage X=%1 Y=%2").arg(m_nImgSizeX).arg(m_nImgSizeY));
    _dibTemp = new QImage(m_nImgSizeX, m_nImgSizeY, QImage::Format_RGB32);
    ...
    for (int32_t nPixY = nRngY1; nPixY < nRngY2; nPixY++) {
        ...
        for (int32_t nPixX = nRngX1; nPixX < nRngX2; nPixX++) {
            ...
            _dibTemp->setPixelColor(nPixX, nPixY, QColor(sPixDst.nFinalR, sPixDst.nFinalG, sPixDst.nFinalB));
        }                           // x
    }                             // y
...
//
void ImgDecode::ChannelExtract(uint32_t nMode, PixelCc &sSrc, PixelCc &sDst) {
...
//
void ImgDecode::CalcChannelPreview() {
    _log.debug("ImgDecode::CalcChannelPreview");
    uint8_t *pDibImgTmpBits = 0;
    ...
    _log.debug(QString("ImgDecode::CalcChannelPreview DibTempReady = %1").arg(_dibTempReady));
...
//
uint32_t ImgDecode::GetPreviewZoomMode() {
    return m_nZoomMode;
}
...
//
uint32_t ImgDecode::GetPreviewMode() {
...
//
double ImgDecode::GetPreviewZoom() {
    return m_nZoom;
}
...
// - m_nZoomMode
//
void ImgDecode::SetPreviewZoom(bool bInc, bool bDec, bool bSet, uint32_t nVal) {
    if (bInc) {
...
//
// void ImgDecode::ViewOnDraw(QPainter *, QRect, QPoint, QFont *, QSize &)
// //void ImgDecode::ViewOnDraw(QPainter * pDC, QRect rectClient, QPoint ptScrolledPos, QFont * pFont, QSize & szNewScrollSize)
// {
//     //  m_nPageWidth = 600;
...
//     //  QRect rectTmp;
//
//     //  _log.debug("ImgDecode::ViewOnDraw Start");
//
...
//     //  if(m_bDibTempReady)
//     //  {
//     //    _log.debug("ImgDecode::ViewOnDraw CP1");
//
...
//     //  {
//     //    // Print label
//     //    _log.debug("Title 2 m_bDibTempReady");
//
//     //    if(!m_bPreviewIsJpeg)
//     //    {
//     //      // For all non-JPEG images, report with simple title
...
//     //    else
//     //    {
//     //      _log.debug("Title 3 m_bPreviewIsJpeg");
//     //      m_strTitle = "Image (";
//
//     //      switch (m_nPreviewMode)
//     //      {
...
// - pDC                        = The device context pointer
//
// void ImgDecode::ViewMcuOverlay(QPainter *pDC) {
//     // Now create overlays
...
//         pDC->drawLine(m_rectImgReal.left() + nXZoomed,
//                       m_rectImgReal.top(),
//                       m_rectImgReal.left() + nXZoomed,
//                       m_rectImgReal.bottom());
//     }
//
//     for (uint32_t nMcuY = 0; nMcuY < m_nMcuYMax; nMcuY++) {
//         nYZoomed = static_cast<int32_t>(nMcuY * m_nMcuHeight * m_nZoom);
//         pDC->drawLine(m_rectImgReal.left(),
//                       m_rectImgReal.top() + nYZoomed,
//                       m_rectImgReal.right(),
//                       m_rectImgReal.top() + nYZoomed);
//     }
//
...
// - pDC                        = The device context pointer
//
// void ImgDecode::ViewMcuMarkedOverlay(QPainter *pDC) {
...
// - nBlkY                      = 8x8 block Y coordinate
//
// void ImgDecode::ViewMarkerOverlay(QPainter *pDC, uint32_t nBlkX, uint32_t nBlkY) {
//     QRect my_rect;
//
//     // Note that drawing is an overlay, so we are dealing with real
//     // pixel coordinates, not preview image coords
...
//
bool ImgDecode::GetPreviewOverlayMcuGrid() {
...
//
void ImgDecode::SetPreviewOverlayMcuGridToggle() {
