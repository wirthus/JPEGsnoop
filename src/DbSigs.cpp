...
// - m_strDbDir
//
DbSigs::DbSigs(ILog &log, SnoopConfig &appConfig) :
    _log(log),
    ...
    QFile sigFile(_appConfig.dbPath() + "Signatures.dat");
    sigFile.open(QIODevice::WriteOnly);
    QDataStream out(&sigFile);
    //  out << m_qSigList;
    _log.debug(QString("Signatures: %1").arg(m_qSigList.count()));
...
void DbSigs::SetFirstRun(bool bFirstRun) {
    m_bFirstRun = bFirstRun;
}

int32_t DbSigs::GetNumSigsInternal() {
    return m_nSigListNum;
}

int32_t DbSigs::GetNumSigsExtra() {
    return m_nSigListExtraNum;
}

// Read an unsigned integer (4B) from the buffer
bool DbSigs::BufReadNum(uint8_t *pBuf, uint32_t &nOut, uint32_t, uint32_t &nOffsetBytes) {
    Q_ASSERT(pBuf);
...
bool DbSigs::BufWriteNum(uint8_t *pBuf, uint32_t nIn, uint32_t, uint32_t &nOffsetBytes) {
    Q_ASSERT(pBuf);
    // TODO: check for buffer bounds
    uint8_t *pBufBase;
    uint32_t *pBufInt;
...
bool DbSigs::BufReadStr(uint8_t *pBuf, QString &strOut, uint32_t nMaxBytes, bool bUni, uint32_t &nOffsetBytes) {
    Q_ASSERT(pBuf);
    QString strOutTmp;
    bool bDone;
...
bool DbSigs::BufWriteStr(unsigned char *pBuf, const QString &strIn, uint32_t nMaxBytes, bool bUni, uint32_t &nOffsetBytes) {
    Q_ASSERT(pBuf);
...
void DbSigs::DatabaseExtraLoad() {
    ...
    if (pInFile.open(QIODevice::ReadOnly) == false) {
        ...
        if (!m_bFirstRun) {
            _log.error(QString("Couldn't find User Signature Database\n\n[%1]\n\nCreating default database").arg(pInFile.errorString()));
    ...
    if (!bFileOk) {
        ...
        // Notify user
        strTmp = QString("Created default User Signature Database. Backup of old DB in [%1]").arg(
            m_strDbDir + QString(DAT_FILE) + ".bak");
    ...
    while (!bDone && bValid) {
        ...
        if (strSec == "*DB*") {
            ...
            for (uint32_t ind = 0; ind < nNumLoad; ind++) {
                ...
                bDbLocalEntryFound = SearchSignatureExactInternal(sDbLocalEntry.strXMake,
                                                                  sDbLocalEntry.strXModel,
                                                                  sDbLocalEntry.strCSig);
...
void DbSigs::DatabaseExtraClean() {
    m_nSigListExtraNum = 0;
    DatabaseExtraStore();
}

uint32_t DbSigs::DatabaseExtraGetNum() {
    return m_nSigListExtraNum;
}

CompSig DbSigs::DatabaseExtraGet(uint32_t nInd) {
    Q_ASSERT(nInd < m_nSigListExtraNum);
    return m_sSigListExtra[nInd];
}

void DbSigs::DatabaseExtraStore() {
    uint8_t *pBuf = nullptr;
    //uint32_t	nBufLenBytes = 0;
    uint32_t nBufOffset = 0;
...
void DbSigs::DatabaseExtraAdd(const QString &strExifMake, const QString &strExifModel,
...
bool DbSigs::SearchSignatureExactInternal(const QString &strMake, const QString &strModel, const QString &strSig) {
...
bool DbSigs::SearchCom(const QString &strCom) {
    bool bFound = false;
...
int32_t DbSigs::GetDBNumEntries() {
    return (m_nSigListNum + m_nSigListExtraNum);
}

// Returns total of built-in plus local DB
uint32_t DbSigs::IsDBEntryUser(uint32_t nInd) {
    if (nInd < m_nSigListNum) {
        return false;
    } else {
        return true;
...
bool DbSigs::GetDBEntry(int32_t nInd, CompSig *pEntry) {
    uint32_t nIndOffset;
...
void DbSigs::SetEntryValid(uint32_t nInd, bool bValid) {
    // TODO: Bounds check
    Q_ASSERT(nInd < m_nSigListExtraNum);
    m_sSigListExtra[nInd].bValid = bValid;
}

uint32_t DbSigs::GetIjgNum() {
    return m_nSwIjgListNum;
}

QString DbSigs::GetIjgEntry(uint32_t nInd) {
    return m_sSwIjgList[nInd];
}

// Update the directory used for user database
void DbSigs::SetDbDir(const QString &strDbDir) {
    m_strDbDir = strDbDir;
}

// Search exceptions for Make/Model in list of ones that don't have Makernotes
bool DbSigs::LookupExcMmNoMkr(const QString &strMake, const QString &strModel) {
    bool bFound = false;
    bool bDone = false;
    uint32_t nInd = 0;
...
bool DbSigs::LookupExcMmIsEdit(const QString &strMake, const QString &strModel) {
    bool bFound = false;
    bool bDone = false;
    uint32_t nInd = 0;
